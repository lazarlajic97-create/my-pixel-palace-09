...
export const Route = createFileRoute("/cancel")({ component: Cancel });
      ...
      <PublicLayout>
        <div className="container-app py-20 max-w-xl text-center">
          ...
          <div className="mt-6 flex justify-center gap-2">
            <Link to="/book" className="btn-primary">Neuen Termin buchen</Link>
      ...
      <div className="container-app py-10 max-w-2xl">
        ...
        <div className="glass-strong p-6 mt-6">
          ...
          <label className="block mt-6">
            ...
            <textarea rows={4} className="input-field resize-none" value={reason} onChange={e => setReason(e.target.value)} />
          </label>

          <div className="mt-6 flex flex-col sm:flex-row gap-2">
            <button onClick={() => setDone(true)} className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl font-semibold text-sm bg-[rgba(248,113,113,0.12)] border border-[rgba(248,113,113,0.3)] text-[color:var(--color-danger)] hover:bg-[rgba(248,113,113,0.18)]">Termin stornieren</button>
            <button onClick={() => navigate({ to: "/reschedule" })} className="btn-secondary">Neuen Termin wählen</button>
          </div>
        </div>
      </div>
    </PublicLayout>
